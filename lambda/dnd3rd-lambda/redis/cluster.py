    ...
    def _execute_command(self, target_node, *args, **kwargs):
        ...
        while ttl > 0:
            ...
            try:
                ...
                redirect_addr = get_node_name(host=e.redis_host, port=e.redis_port)
                asking = True
...
class NodesManager:
    ...
    def _update_moved_slots(self):
        ...
        redirected_node = self.get_node(host=e.redis_host, port=e.redis_port)
        if redirected_node is not None:
            ...
            redirected_node = ClusterNode(e.redis_host, e.redis_port, PRIMARY)
        ...
        for node in nodes:
            if node.redis_connection is None:
                node.redis_connection = self.create_redis_node(
                    host=node.redis_host, port=node.redis_port, **self.connection_kwargs
                )
    ...
    def initialize(self):
        ...
        for startup_node in self.startup_nodes.values():
            try:
                if startup_node.redis_connection:
                    ...
                    r = self.create_redis_node(
                        startup_node.redis_host, startup_node.redis_port, **kwargs
                    )
            ...
            if (
                ...
                cluster_slots[0][2][0] = startup_node.redis_host

            for slot in cluster_slots:
                primary_node = slot[2]
                host = str_if_bytes(primary_node[0])
                if host == "":
                    host = startup_node.redis_host
